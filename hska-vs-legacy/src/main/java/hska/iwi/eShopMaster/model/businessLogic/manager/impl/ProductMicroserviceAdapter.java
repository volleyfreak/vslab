...
public class ProductMicroserviceAdapter implements ProductManager {
    ...
    public List<Product> getProductsForSearchValues(String searchValue, Double searchMinPrice, Double searchMaxPrice) {
        ...
        if (searchValue == null)
            searchValue = "";
        Product[] productArray = new RestTemplate().getForObject("http://product:8082/products?name=" + searchValue + "&min=" + Double.toString(searchMinPrice) + "&max=" + Double.toString(searchMaxPrice), Product[].class);
        return new ArrayList<Product>(Arrays.asList(productArray));
    }

    public boolean deleteProductsByCategoryId(int categoryId) {
        new RestTemplate().delete("http://product:8082/category/{id}", categoryId);
        return true;
    }
